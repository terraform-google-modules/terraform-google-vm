...
resource "google_compute_disk_resource_policy_attachment" "attachment" {
  ...
  project = element(split("/", var.disks[count.index]), index(split("/", var.disks[count.index]), "projects", ) + 1, )
  disk    = element(split("/", var.disks[count.index]), index(split("/", var.disks[count.index]), "disks", ) + 1, )
  zone    = element(split("/", var.disks[count.index]), index(split("/", var.disks[count.index]), "zones", ) + 1, )
