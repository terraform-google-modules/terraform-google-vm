...
| autoscaling\_scale\_in\_control | Autoscaling, scale-in control block as single element array. https://www.terraform.io/docs/providers/google/r/compute_autoscaler.html#scale_in_control | <pre>list(object({<br>    max_scaled_in_replicas = list(object({<br>      fixed   = number<br>      percent = number<br>    }))<br>    time_window_sec = number<br>  }))</pre> | `[]` | no |
