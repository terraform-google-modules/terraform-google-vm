...
| stateful\_external\_ips | External IPs created on the instances that will be preserved on instance delete. https://cloud.google.com/compute/docs/instance-groups/configuring-stateful-ip-addresses-in-migs | <pre>list(object({<br>    network_interface_name = string<br>    delete_rule            = string<br>  }))</pre> | `[]` | no |
| stateful\_internal\_ips | Internal IPs created on the instances that will be preserved on instance delete. https://cloud.google.com/compute/docs/instance-groups/configuring-stateful-ip-addresses-in-migs | <pre>list(object({<br>    network_interface_name = string<br>    delete_rule            = string<br>  }))</pre> | `[]` | no |
