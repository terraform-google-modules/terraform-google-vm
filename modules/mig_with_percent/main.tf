...
resource "google_compute_region_instance_group_manager" "mig_with_percent" {
  ...
  name   = var.mig_name == "" ? "${var.hostname}-mig-with-percent" : var.mig_name
  region = var.region
  dynamic "named_port" {
  ...
  dynamic "stateful_internal_ip" {
    ...
    content {
      device_name = stateful_internal_ip.value.network_interface_name
  ...
  dynamic "stateful_external_ip" {
    ...
    content {
      device_name = stateful_external_ip.value.network_interface_name
