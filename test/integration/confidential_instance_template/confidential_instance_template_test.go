...
func TestConfidentialInstanceTemplate(t *testing.T) {
	const instanceNamePrefix = "confidential-template"

	confInsTempl := tft.NewTFBlueprintTest(t)
	confInsTempl.DefineVerify(func(assert *assert.Assertions) {
		confInsTempl.DefaultVerify(assert)
