...
control "MIG" do
  title "Autoscaling Configuration with Scale Down Control"

  describe command("gcloud --project=#{project_id} compute instances list --format=json --filter='name~^mig-scale-down*'") do
  ...
  describe command("gcloud --project=#{project_id} compute instance-groups list --format=json --filter='name~^mig-scale-down*'") do
    its(:exit_status) { should eq 0 }
    its(:stderr) { should eq '' }
  ...
  describe command("gcloud beta --project=#{project_id} compute instance-groups managed list --format=json --filter='name~^mig-scale-down*'") do
    its(:exit_status) { should eq 0 }
    its(:stderr) { should eq '' }
    ...
    describe "autoscaler scaling policy" do
      it "maxScaledDownReplicas should be 1" do
        expect(data[0]['autoscaler']['autoscalingPolicy']['scaleDownControl']['maxScaledDownReplicas']['fixed']).to eq(1)
      end
    end
  end
