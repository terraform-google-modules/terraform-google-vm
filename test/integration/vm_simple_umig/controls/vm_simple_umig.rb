...
ENV['CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE'] = File.absolute_path(
  credentials_path,
  File.join(__dir__, "../../../fixtures/vm/simple_umig"))
...
control "VM" do
  ...
  describe command("gcloud --project=#{project_id} compute instances list --format=json --filter='name~vm-simple-umig*'") do
  ...
  describe command("gcloud --project=#{project_id} compute instance-groups list --format=json --filter='name~vm-simple-umig*'") do
    its(:exit_status) { should eq 0 }
    its(:stderr) { should eq '' }
